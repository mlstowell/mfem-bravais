...
int main(int argc, char *argv[])
{
   ...
   */
   ...
   vector<ParGridFunction*> x(nev);
   HypreParVector ** init_vecs = new HypreParVector*[nev];

   for (int i=0; i<nev; i++)
   {
     x[i] = new ParGridFunction(fespace);
     init_vecs[i] = NULL;
   }
   
   for (int lev = -1; lev < par_ref_levels; lev++)
   {
     HYPRE_Int size = fespace->GlobalTrueVSize();
     if (myid == 0)
     {
     ...
     if ( lev > 0 )
     {
       for (int i=0; i<nev; i++)
	 {
	   if ( init_vecs[i] != NULL ) delete init_vecs[i];
   ...
   // 12. Free the used memory.
     ...
     if ( lev < par_ref_levels - 1 )
     {
       cout << "Refining mesh and updating objects" << endl;
       pmesh->UniformRefinement();
	 ...
	 x[i]->Update();
	 if ( init_vecs[i] != NULL ) delete init_vecs[i];
	 init_vecs[i] = x[i]->ParallelProject();
	 if ( init_vecs[i] == NULL )
	   cout << "init_vecs[" << i << "] is NULL" << endl;
	 else
	   cout << "init_vecs[" << i << "] is not NULL" << endl;
       }
     }
   }
   delete a;
