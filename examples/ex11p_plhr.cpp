...
int main(int argc, char *argv[])
{
   ...
   PLHRSolver * plhr = new PLHRSolver(MPI_COMM_WORLD);
   plhr->SetMaxIter(200);
   plhr->SetRelTol(1e-8);
   plhr->SetOperator(*A, *M);
...

   {
      ...
      for (int i=0; i<nev; i++)
      {
         double shift = eigenvalues[i] + eps;
         ...
         */
         ...
         plhr->SetPreconditioner(gmres);
         plhr->SetShift(shift);
         plhr->Reinitialize();

         double eig_plhr = 0.0;

         plhr->GetEigenPair(eig_plhr, X);

         delete C;

         if ( myid == 0 )
         {
            cout << i << ":\t" << eigenvalues[i] << "\t" << eig_plhr << endl;
         }
