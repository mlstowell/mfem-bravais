...
#include "maxwell_bloch_krylov.hpp"
#include <fstream>
...
MaxwellBlochWaveEquationKrylov::MaxwellBlochWaveEquationKrylov(ParMesh & pmesh,
                                                               int order)
   : myid_(0),
     nev_(-1),
     newBeta_(true),
...
MaxwellBlochWaveEquationKrylov::~MaxwellBlochWaveEquationKrylov()
{
   delete lobpcg_;
...
void
MaxwellBlochWaveEquationKrylov::SetKappa(const Vector & kappa)
{
   if ( myid_ == 0 )
   {
      cout << "Setting Kappa: ";
...
void
MaxwellBlochWaveEquationKrylov::SetBeta(double beta)
{
   beta_ = beta; newBeta_ = true;
}

void
MaxwellBlochWaveEquationKrylov::SetZeta(const Vector & zeta)
{
   zeta_ = zeta; newZeta_ = true;
}

void
MaxwellBlochWaveEquationKrylov::SetAbsoluteTolerance(double atol)
{
   atol_ = atol;
}

void
MaxwellBlochWaveEquationKrylov::SetNumEigs(int nev)
{
   nev_ = nev;
}

void
MaxwellBlochWaveEquationKrylov::SetMassCoef(Coefficient & m)
{
   mCoef_ = &m; newMCoef_ = true;
}

void
MaxwellBlochWaveEquationKrylov::SetStiffnessCoef(Coefficient & k)
{
   kCoef_ = &k; newKCoef_ = true;
}

void
MaxwellBlochWaveEquationKrylov::SetMaximumLightSpeed(double c)
{
...
void
MaxwellBlochWaveEquationKrylov::Setup()
{
   if ( newKCoef_ )
   {
   ...
   if ( newZeta_ || newBeta_ || newKCoef_ )
   {
      ...
      if ( fabs(beta_) > 0.0 )
      {
         ...
         dynamic_cast<MINRESSolver*>(T1Inv_minres_)->SetAbsTol(1.0e-3);
         dynamic_cast<MINRESSolver*>(T1Inv_minres_)->SetRelTol(1.0e-3);
         dynamic_cast<MINRESSolver*>(T1Inv_minres_)->SetMaxIter(1000);
         dynamic_cast<MINRESSolver*>(T1Inv_minres_)->SetPrintLevel(false);
   ...
   if ( ( newZeta_ || newBeta_ || newMCoef_ || newKCoef_ ) && nev_ > 0 )
   {
      if ( fabs(beta_) > 0.0 )
      {
         ...
         SubSpaceProj_ = new MaxwellBlochWaveProjectorKrylov(*HCurlFESpace_,
                                                             *H1FESpace_,
                                                             *M_,beta_,zeta_);
         SubSpaceProj_->Setup();
...
void
MaxwellBlochWaveEquationKrylov::SetInitialVectors(int num_vecs,
                                                  HypreParVector ** vecs)
{
   if ( lobpcg_ )
   {
...
void MaxwellBlochWaveEquationKrylov::Update()
{
   if ( myid_ == 0 ) { cout << "Building M2(k)" << endl; }
   ParBilinearForm m2(HDivFESpace_);
   m2.AddDomainIntegrator(new VectorFEMassIntegrator(*kCoef_));
...
void
MaxwellBlochWaveEquationKrylov::Solve()
{
   if ( nev_ > 0 )
   {
      if ( fabs(beta_) > 0.0 )
...
void
MaxwellBlochWaveEquationKrylov::GetEigenvalues(vector<double> & eigenvalues)
{
   if ( lobpcg_ )
   {
      Array<double> eigs;
...
void
MaxwellBlochWaveEquationKrylov::GetEigenvalues(int nev, const Vector & kappa,
...
void
MaxwellBlochWaveEquationKrylov::GetEigenvector(unsigned int i,
                                               HypreParVector & Er,
                                               HypreParVector & Ei,
                                               HypreParVector & Br,
                                               HypreParVector & Bi)
...
void
MaxwellBlochWaveEquationKrylov::GetEigenvectorE(unsigned int i,
                                                HypreParVector & Er,
                                                HypreParVector & Ei)
{
   double * data = NULL;
...
void
MaxwellBlochWaveEquationKrylov::GetEigenvectorB(unsigned int i,
                                                HypreParVector & Br,
                                                HypreParVector & Bi)
{
   vector<double> eigenvalues;
...
void
MaxwellBlochWaveEquationKrylov::GetFourierCoefficients(HypreParVector & Vr,
                                                       HypreParVector & Vi,
                                                       Array2D<double> &f)
{
   f = 0.0;
...
void
MaxwellBlochWaveEquationKrylov::GetFieldAverages(unsigned int i,
                                                 Vector & Er, Vector & Ei,
                                                 Vector & Br, Vector & Bi,
                                                 Vector & Dr, Vector & Di,
                                                 Vector & Hr, Vector & Hi)
{
   ...
   {
      MFEM_ASSERT(bravais_ != NULL, "MaxwellBlochWaveEquationKrylov: "
...
void
MaxwellBlochWaveEquationKrylov::WriteVisitFields(const string & prefix,
                                                 const string & label)
{
...
MaxwellBlochWaveEquationKrylov::MaxwellBlochWavePrecond::
...
MaxwellBlochWaveEquationKrylov::
...
MaxwellBlochWaveProjectorKrylov::
MaxwellBlochWaveProjectorKrylov(ParFiniteElementSpace & HCurlFESpace,
...
MaxwellBlochWaveProjectorKrylov::~MaxwellBlochWaveProjectorKrylov()
{
...
void
MaxwellBlochWaveProjectorKrylov::SetBeta(double beta)
{
   beta_ = beta; newBeta_ = true;
}

void
MaxwellBlochWaveProjectorKrylov::SetZeta(const Vector & zeta)
{
   zeta_ = zeta; newZeta_ = true;
}

void
MaxwellBlochWaveProjectorKrylov::Setup()
{
   if ( myid_ == 0 )
   {
...
void
MaxwellBlochWaveProjectorKrylov::Update()
{
...
void
MaxwellBlochWaveProjectorKrylov::Mult(const Vector &x, Vector &y) const
{
   M_->Mult(x,y);
