...
int main(int argc, char *argv[])
{
   ...
   args.AddOption(&sns, "-sns", "--shift-null-space", "-no-sns",
                  "--no-shift-null-space",
                  "Shift the null space rather than project it out.");
   ...
   MaxwellBlochWaveEquationShift * eq =
      new MaxwellBlochWaveEquationShift(*pmesh, order, sns);
   ...
   eq->SetMaximumLightSpeed(1.0);

   // DenseMatrix dispersion(num_beta,nev);

   ostringstream oss_prefix;
   oss_prefix << "Maxwell-Dispersion-Shift-";
   if ( sns ) { oss_prefix << "SNS-"; }
