     ...
     // energy_(NULL)
{
   ...
   oneLF_ = new ParLinearForm(L2FESpace_);
   oneLF_->AddDomainIntegrator(new DomainLFIntegrator(oneCoef_));
   oneLF_->Assemble();
   /*
...
void
MaxwellBlochWaveEquationAMR::SetEpsilonCoef(Coefficient & eps)
{
   epsCoef_ = &eps; newMCoef_ = true;
   delete epsInvCoef_; epsInvCoef_ = new PowerCoefficient(*epsCoef_, -1.0);

   if ( epsCoefGF_ == NULL )
   {
      epsCoefGF_ = new ParGridFunction(L2FESpace_);
   }
   epsCoefGF_->ProjectCoefficient(*epsCoef_);
   epsAvg_ = oneLF_->operator()(*epsCoefGF_);
...
*/
void
MaxwellBlochWaveEquationAMR::SetMuCoef(Coefficient & mu)
{
   muCoef_ = &mu; newACoef_ = true;
   delete muInvCoef_; muInvCoef_ = new PowerCoefficient(*muCoef_, -1.0);

   if ( muCoefGF_ == NULL )
   {
      muCoefGF_ = new ParGridFunction(L2FESpace_);
   }
   muCoefGF_->ProjectCoefficient(*muCoef_);
   muAvg_ = oneLF_->operator()(*muCoefGF_);
   ...
   if ( ! currD12_ )
   {
      ...
      if ( newACoef_ )
      {
         ...
         d12_ = new ParMixedBilinearForm(HCurlFESpace_,
                                         HCurlFESpace_);
         d12_->AddDomainIntegrator(new MixedVectorCurlIntegrator);
      }
      else
      {
         d12_->Update();
...
MaxwellBlochWaveEquationAMR::SetupM1()
{
   ...
   if ( ! currM1_ )
   {
      ...
      m1.AddDomainIntegrator(new VectorFEMassIntegrator(*epsCoef_));
...
MaxwellBlochWaveEquationAMR::SetupM2()
{
   ...
   if ( ! currM2_ )
   {
      ...
      m2.AddDomainIntegrator(new VectorFEMassIntegrator(*muCoef_));
...
MaxwellBlochWaveEquationAMR::SetupBlockOperatorA12()
{
   ...
   if ( A12_ == NULL || newSizes_ )
   {
      ...
      A12_->owns_blocks = false;
   }
   if ( newZeta_ || newBeta_ || newACoef_ || newSizes_ )
   {
      A12_->SetBlock(0, 3, D12_,  1.0);
      A12_->SetBlock(1, 2, D12_, -1.0);
      A12_->SetBlock(2, 1, D12_, -1.0);
      A12_->SetBlock(3, 0, D12_,  1.0);
      if ( fabs(beta_) > 0.0 )
      {
         A12_->SetBlock(0, 2, Z12_, -beta_);
         A12_->SetBlock(1, 3, Z12_, -beta_);
...
MaxwellBlochWaveEquationAMR::SetupBlockSolver()
{
   ...
   */
   ...
   // double lambda0 = 2.6;
   cout << "|k|min = " << sqrt(kSqrMin) << ", |k|max = " << sqrt(kSqrMax) << endl;
   cout << "averaged speed of light: " << 1.0/sqrt(epsAvg_ * muAvg_) << endl;

   double lambda0 = 0.5 * (sqrt(kSqrMin) + sqrt(kSqrMax)) / sqrt(epsAvg_ * muAvg_);
   //double lambda0 = 4.2;
   ...
   */
   ConstantCoefficient om2Coef(omega_shift_ * omega_shift_);
   ProductCoefficient  om2epsCoef(om2Coef, *epsCoef_);
   ProductCoefficient  om2muCoef(om2Coef, *muCoef_);

   VectorFunctionCoefficient zmuInvCoef(zeta_, *muInvCoef_);
   VectorFunctionCoefficient zepsInvCoef(zeta_, *epsInvCoef_);

   DenseMatrix kk(3); // - k x k x
   ...
   MatrixFunctionCoefficient negkxmuInvkxCoef(kk, *muInvCoef_);
   MatrixFunctionCoefficient negkxepsInvkxCoef(kk, *epsInvCoef_);

   delete t1_;
   t1_ = new ParBilinearForm(HCurlFESpace_);
   t1_->AddDomainIntegrator(new VectorFEMassIntegrator(om2epsCoef));
   t1_->AddDomainIntegrator(new VectorFEMassIntegrator(negkxmuInvkxCoef));
   t1_->AddDomainIntegrator(new CurlCurlIntegrator(*muInvCoef_));
   t1_->Assemble();
   t1_->Finalize();
   ...
   delete t1i_;
   t1i_ = new ParBilinearForm(HCurlFESpace_);
   t1i_->AddDomainIntegrator(new MixedCrossCurlIntegrator(zmuInvCoef));
   t1i_->AddDomainIntegrator(new MixedWeakCurlCrossIntegrator(zmuInvCoef));
   t1i_->Assemble();
   t1i_->Finalize();
   delete T1i_; T1i_ = t1i_->ParallelAssemble();
   T1i_->Print("T1i.mat");
   ...
   t2_->AddDomainIntegrator(new VectorFEMassIntegrator(om2muCoef));
   t2_->AddDomainIntegrator(new VectorFEMassIntegrator(negkxepsInvkxCoef));
   t2_->AddDomainIntegrator(new CurlCurlIntegrator(*epsInvCoef_));
   t2_->Assemble();
   t2_->Finalize();
   ...
   delete t2i_;
   t2i_ = new ParBilinearForm(HCurlFESpace_);
   t2i_->AddDomainIntegrator(new MixedCrossCurlIntegrator(zepsInvCoef));
   t2i_->AddDomainIntegrator(new MixedWeakCurlCrossIntegrator(zepsInvCoef));
   t2i_->Assemble();
   t2i_->Finalize();
   delete T2i_; T2i_ = t2i_->ParallelAssemble();
   T2i_->Print("T2i.mat");
   ...
   delete B12_; B12_ = new BlockOperator(block_trueOffsets12_);
   B12_->owns_blocks = false;
   B12_->SetDiagonalBlock(0, T1_);
   B12_->SetDiagonalBlock(1, T1_);
   B12_->SetDiagonalBlock(2, T2_);
   B12_->SetDiagonalBlock(3, T2_);
   B12_->SetBlock(0, 1, T1i_,  beta_);
   B12_->SetBlock(1, 0, T1i_, -beta_);
   B12_->SetBlock(2, 3, T2i_,  beta_);
   B12_->SetBlock(3, 2, T2i_, -beta_);
   B12_->SetBlock(0, 3, D12_, -2.0 * omega_shift_);
   B12_->SetBlock(1, 2, D12_,  2.0 * omega_shift_);
   B12_->SetBlock(2, 1, D12_,  2.0 * omega_shift_);
   B12_->SetBlock(3, 0, D12_, -2.0 * omega_shift_);
   B12_->SetBlock(0, 2, Z12_,  2.0 * omega_shift_ * beta_);
   B12_->SetBlock(1, 3, Z12_,  2.0 * omega_shift_ * beta_);
   B12_->SetBlock(2, 0, Z12_, -2.0 * omega_shift_ * beta_);
   B12_->SetBlock(3, 1, Z12_, -2.0 * omega_shift_ * beta_);
   ...
   */
   if ( init_vecs_ == NULL )
   {
      ...
      HypreParVector tmp(comm_, 4 * hcurl_glb_size_,
                         part_);

      cout << "Allocating init_vecs_" << endl;
   ...
   lobpcg_ = new HypreLOBPCG(comm_);
   lobpcg_->SetNumModes(nev_);
   lobpcg_->SetMaxIter(1000);
   // lobpcg_->SetTol(atol_);
   lobpcg_->SetTol(1.0e-6);
   lobpcg_->SetPrintLevel(1);
                                              ...
                                              Vector & Hr, Vector & Hi)
{
   ...
   ConstantCoefficient * aConst = dynamic_cast<ConstantCoefficient*>(muInvCoef_);
   if ( aConst != NULL )
   {
                                              ...
                                              const string & label)
{
   ...
   if ( dynamic_cast<GridFunctionCoefficient*>(epsCoef_) )
   {
      GridFunctionCoefficient * gfc =
         dynamic_cast<GridFunctionCoefficient*>(epsCoef_);
      visit_dc.RegisterField("epsilon", gfc->GetGridFunction() );
   }
   else
   {
      epsCoefGF_->ProjectCoefficient(*epsCoef_);
      visit_dc.RegisterField("epsilon", epsCoefGF_);
   }
   if ( dynamic_cast<GridFunctionCoefficient*>(muCoef_) )
   {
      GridFunctionCoefficient * gfc =
         dynamic_cast<GridFunctionCoefficient*>(muCoef_);
      visit_dc.RegisterField("mu", gfc->GetGridFunction() );
   }
   else
   {
      muCoefGF_->ProjectCoefficient(*muCoef_);
      visit_dc.RegisterField("mu", muCoefGF_);
   }
                                            ...
                                            int offx, int offy)
{
   ...
   VisualizeField(m_sock, vishost, visport, *epsCoefGF_,
                  "Epsilon Coefficient", Wx, Wy, Ww, Wh);
   VisualizeField(a_sock, vishost, visport, *muCoefGF_,
                  "Mu Coefficient", Wx, Wy+offy, Ww, Wh);
}
