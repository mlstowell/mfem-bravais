...
MaxwellBlochWaveEquationAMR::SetupSizes()
{
   ...
   if ( ! currSizes_ )
   {
      ...
      block_trueOffsets12_[3] = HDivFESpace_->TrueVSize();
      block_trueOffsets12_[4] = HDivFESpace_->TrueVSize();
      ...
      int locSize = 2 * (hcurl_loc_size_ + hdiv_loc_size_);
...
MaxwellBlochWaveEquationAMR::SetupBlockSolver()
{
   ...
   */
   ...
   delete t2_;
   t2_ = new ParBilinearForm(HDivFESpace_);
   t2_->AddDomainIntegrator(new VectorFEMassIntegrator(taCoef));
   // t2_->AddDomainIntegrator(new CurlCurlIntegrator(*mCoef_));
   t2_->Assemble();
   t2_->Finalize();
   ...
   if ( true && num_procs_ == 1 )
   {
      // Test preconditioner
      HypreParVector tmpa(comm_, 2 * (hcurl_glb_size_ +
                                      hdiv_glb_size_),
                          part_);
      HypreParVector tmpb(comm_, 2 * (hcurl_glb_size_ +
                                      hdiv_glb_size_),
                          part_);
      HypreParVector tmpc(comm_, 2 * (hcurl_glb_size_ +
                                      hdiv_glb_size_),
                          part_);
      tmpa = 0.0;
      tmpc = 0.0;

      int size = 2 * (hcurl_glb_size_ + hdiv_glb_size_);
      // double maxDiff = 0.0;
   ...
   */
   ...
   while ( it <= ar_ )
   {
      ...
      if ( it > 0 )
      {
         ...
         if ( init_gfr_ == NULL )
         {
            init_gfr_ = new ParGridFunction*[num_init_vecs_];
            for ( int i=0; i<num_init_vecs_; i++)
            {
               init_gfr_[i] = new ParGridFunction(HCurlFESpace_);
            }
         }
         if ( init_gfi_ == NULL )
         {
            init_gfi_ = new ParGridFunction*[num_init_vecs_];
            for ( int i=0; i<num_init_vecs_; i++)
            {
               init_gfi_[i] = new ParGridFunction(HCurlFESpace_);
            }
         }
         for (int i=0; i<num_init_vecs_; i++)
         {
         ...
         /*
              ParGridFunction ** vr = new ParGridFunction*[num_init_vecs_];
              ParGridFunction ** vi = new ParGridFunction*[num_init_vecs_];

              HypreParVector Vr(this->GetHCurlFESpace()->GetComm(),
                                this->GetHCurlFESpace()->GlobalTrueVSize(),
                                NULL,
                                this->GetHCurlFESpace()->GetTrueDofOffsets());
              HypreParVector Vi(this->GetHCurlFESpace()->GetComm(),
                                this->GetHCurlFESpace()->GlobalTrueVSize(),
                                NULL,
                                this->GetHCurlFESpace()->GetTrueDofOffsets());
              ...
              for (int i=0; i<num_init_vecs_; i++)
              {
                 vr[i] = new ParGridFunction(HCurlFESpace_);
                 vi[i] = new ParGridFunction(HCurlFESpace_);

                 Vr.SetData(&(*init_vecs_[i])(0));
                 Vi.SetData(&(*init_vecs_[i])(hcurl_loc_size_));
         ...
         */
         ...
         for (int i=0; i<num_init_vecs_; i++)
         {
            ...
            Ei.SetDataAndSize(&(*init_vecs_[i])(hcurl_loc_size_),
                              hcurl_loc_size_);

            init_gfr_[i]->ParallelAssemble(Er);
            init_gfi_[i]->ParallelAssemble(Ei);
         }
         if ( lobpcg_ )
         {
            lobpcg_->SetInitialVectors(num_init_vecs_, init_vecs_);
